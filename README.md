...
- `/reps/[user_key]` — calls KPIs + trend + Top HCPs (existing) + **Net Units card + sales trend + Top HCOs by Units** + **Coverage HCOs** section (multi-visibility, all bridged HCOs with Primary/Co-coverage badges per Option B hybrid model). **"Suggested this week" LLM card** with 3-5 prioritized HCP/HCO recommendations + per-row "Show context" expand panel (affiliated HCPs at the HCO + sales mini-trend for HCO suggestions; parent HCO + recent calls for HCP suggestions). Cached per (rep × pipeline_run), 4h rate-limit. Tier-aware weighting in the prompt; underactive coverage (HCOs in book with zero calls via affiliated HCPs in last 8 weeks) as a primary input. **Action launchpad** in each expanded row: **Open in Veeva** (deep link to `https://<vault_domain>/ui/#object/account__v/<veeva_account_id>` for Veeva Vault tenants — pattern needs per-tenant config when CRM-on-Salesforce tenants land) + **Generate call brief** (on-demand LLM-generated 4-6 bullet pre-call brief blending targeting score, recent calls by this rep, sales motion, and per-scope contributors; cached per (rep × entity × pipeline_run) with 4h rate-limit).
- `/hcps/[hcp_key]` — calls-focused detail (sales rarely attributed at HCP grain in 867 data). **"Since your last visit" panel** at top: deterministic diff anchored on the viewer's most-recent call (or tenant-wide most-recent when viewer has none) — surfaces parent-HCO sales motion since anchor (with delta% vs equivalent prior window), other reps who've called in the interval, and first-ever-sale flags. **"Compared to similar HCPs" panel** below the targeting score: descriptive cohort comparison (same tier × specialty × composite-score 20-pt band) showing this HCP's 90d call count vs cohort median, channel mix breakdown of cohort calls, and a rising-prescribing subset (cohort HCPs with rising parent-HCO units) for "what engagement looks like on similar HCPs that are growing." Strictly descriptive — never claims causation; correlation footer reinforces.
...
**Rep action launchpad SHIPPED 2026-04-29.** Surface C v2 from `project_rep_action_paradigm` memory. Two action buttons in each expanded recommendation row:
- **Open in Veeva** — direct deep link using tenant_veeva.vault_domain + dim_hcp/dim_hco.veeva_account_id. Vault-domain-aware so each tenant's Veeva CRM instance routes correctly. Opens in new tab.
- **Generate call brief** — on-demand server action backed by `lib/call-brief.ts`. Inputs blend: entity meta (HCP: name, specialty, tier, parent HCO; HCO: name, type, tier, location), full targeting score breakdown (composite + per-scope + top contributors), this rep's last 5 calls to the entity, sales motion (last 90 vs prior 90 net units + last sale date), and (for HCOs) top 5 affiliated HCPs by score. LLM produces 4-6 bullets ranked by importance. Cache: per (tenant × rep × entity_kind × entity_key × pipeline_run_id) with 4-hour rate-limit floor across regenerations. New `call_brief_cache` Postgres table; migration `0012_melodic_tarantula.sql` applied.

Per `project_rep_action_paradigm` deliberately NOT a "Mark as called" or other state-tracking — Veeva is the source of truth for calls.

**Next major lift:** TBD. See "Still open" below + `project_v2_product_backlog` memory for queued items (Tests, Mapping kinds beyond account_xref, Email digest, "Find similar HCPs" follow-up to launchpad).
